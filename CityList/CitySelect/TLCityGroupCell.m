...
- (void) setCityArray:(NSArray *)cityArray
{
    ...
    for (int i = 0; i < cityArray.count; i ++) {
        ...
        if ([cityArray.firstObject isKindOfClass:[NSDictionary class]]) {
            NSDictionary *cityDic = [cityArray objectAtIndex:i];
            [button setTitle:[cityDic objectForKey:@"name"] forState:UIControlStateNormal];
        }
        if ([cityArray.firstObject isKindOfClass:[NSString class]]) {
            NSString *citystr = [cityArray objectAtIndex:i];
...
{
    id city = [self.cityArray objectAtIndex:sender.tag];
